...
def emission_to_sentence(emission, id_map):
    return ' '.join([id_map[word] for word in emission]).capitalize()


def parse_syllables(syllables_file):
    word_corpus = set()
    word_to_syllable = defaultdict(lambda: [])
    pattern = re.compile(r"[^\w']+", re.UNICODE)
    with open(syllables_file, 'r') as f:
        for line in f:
            word_info = line.strip().split()
            word = pattern.sub('', word_info[0]).lower()
            word_corpus.add(word)
            for i in range(1, len(word_info)):
                word_to_syllable[word].append(word_info[i])
